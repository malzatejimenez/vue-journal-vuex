<template>
  ...
  <img v-else-if="entry.picture" :src="entry.picture" alt="entry-picture" class="img-thumbnail" />
</template>
...
export default {
  ...
  components: {
    Fab: defineAsyncComponent(() => import(/* webpackChunkName: "daybook" */ "@/modules/daybook/components/Fab")),
  },
  ...
  methods: {
    ...
    async onDeleteEntry() {
      ...
      if (isConfirmed) {
        ...
        Swal.showLoading();
        await this.deleteEntry(this.entry.id).then(() => this.$router.push({ name: "no-entry" }));
